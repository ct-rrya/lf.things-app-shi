-- Drop and recreate found_items table with all required columns
-- WARNING: This will delete all existing found item reports!
...
CREATE INDEX idx_found_items_reporter_id ON found_items(reporter_id);
...
DROP POLICY IF EXISTS "Authenticated users can report found items" ON found_items;
DROP POLICY IF EXISTS "Users can view their own found item reports" ON found_items;
DROP POLICY IF EXISTS "Users can update their own found item reports" ON found_items;
DROP POLICY IF EXISTS "Item owners can view matched found items" ON found_items;
DROP POLICY IF EXISTS "Admin users can view all found items" ON found_items;
DROP POLICY IF EXISTS "Admin users can update all found items" ON found_items;
