 ...
 * @param {string} params.targetId - ID of the affected entity (UUID for items, TEXT for students)
...
export async function logAuditAction({
  ...
  try {
    ...
    // Check if targetId is a valid UUID
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    const isUUID = targetId && uuidRegex.test(targetId);

    // For non-UUID identifiers (like student_id), store in new_value and use NULL for target_id
    const auditData = {
      actor_id: actor,
      action,
      ...
      old_value: oldValue,
      new_value: isUUID ? newValue : { ...newValue, _identifier: targetId },
    };
